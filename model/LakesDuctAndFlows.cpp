/*
 * LakesDuctAndFlows.cpp
 *
 *  Created on: Dec 7, 2015
 *      Author: laetus
 */

#include "LakesDuctAndFlows.hpp"

using namespace casadi;

LakesDuctAndFlows::LakesDuctAndFlows(int nTimesteps, int horizon, int n_f) :
	LakesDuctsAndFlows::HydroModel(nTimesteps, horizon, n_f) {
	this->t = SX::sym("time");
	this->name = "LakesDuctAndFlows";
...
Matrix<double> LakesDuctAndFlows::waterMass() {
...
casadi::MX LakesDuctAndFlows::getf(int t_act) {
	//	MX result;
	...
	//
	//	double lambda = 1;
	//	result += lambda * mul(transpose((V * selector) - shift), (V * selector)
	//			- shift);
	...
	if (isRealtime) {
		...
		shift[pump10->getPlace()] = pump10->getGlobalOpt()[Slice(t_act, t_act
				+ pump10->getSpaceNeeded())];

		result = mul(transpose((V * selector) - shift),
				((V * selector) - shift));
		...
		powerPrice_tact = 1e-4 * powerPrice[Slice(t_act, t_act + nTimesteps)];
		producedPower_should = Matrix<double>::zeros(nTimesteps, 1);
		result = mul(-transpose(producedPower_tact - producedPower_should),
				intervall_length_in_h * powerPrice_tact);
